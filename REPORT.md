...
Et tant qu'à faire, la validité des arguments qui ne sont pas un fichier sera également validé.
Cette validation permettra également d'éviter de passer à `nmap` la nom de fichier spécial `-` qui est indique l'input standard
...
L'option verbose `-v` permet d'afficher encore plus d'information en temps réel, comme l'estimation du temps restant, qui me parait très important comme retour à avoir.
Le désavantage et qu'il ajoute aussi des informations moins utiles comme les machines détectée hors ligne.
...
- `-A` (équivalent à `-sV -sC -O --traceroute` (version detection, Nmap Scripting Engine with the default set of scripts, remote OS detection, and traceroute))
  ...
  Option classique, elle permet de recueillir un maximum d'information lorsque l'on s'intéresse entre autre à la découverte de vulnérabilités (ce qui est notre cas ici)
  ...
  sont ajoutés grâce aux scripts NSE de la catégorie `default` qui sont lancés.
...
- `-T aggressive`
  ...
  J'ai choisi celle-ci car il s'agit de l'option recommandée dans le livre lorsque l'on peut faire à peu près confiance 
  à la rapidité et la fiabilité de la connexion avec les cibles et qu'on ne cherche pas à être particulièrement furtif.
...
Procéder à deux scans séparés aurait été possible mais aurait grandement complexifié le script en ajoutant le besoin de fusionner ensuite les deux résultats dans un seul rendu `HTML`.
...
`nmap` ne permet malheureusement pas de se servir des options `-F` ou `--top-ports` pour un protocole seulement, sinon je les aurais utilisées pour réduire le nombre de port TCP scannés et pour ne pas avoir à écrire en dur les port UDP voulus.
...
Il faut donc choisir un liste de *probes* avec là aussi un compromis à faire entre le nombre d'IP correctement détectées en ligne et le temps pris par cette vérification.
...
`nmap-vulners` a l'air d'être déjà présent dans les scripts fourni avec `nmap` sous le nom de `vulners` mais la version du github a l'air plus complète que celle sur ma machine.
...
J'ai pensé à faire un container Docker, mais le fait de devoir passé des arguments différents à chaque foit n'aurait pas rendu cela pratique.
